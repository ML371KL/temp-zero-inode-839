...
function alertChipMarkup(rule) {
  ...
  // по ней срабатывает. Не появляется и закрытая биржа: сервер стреляет по любому
  // новому принту, хоть на премаркете, хоть овернайт. Остаётся ровно одно — фид,
  // у которого метка цены не двигается.
  else if (blocked) {
    tone = "is-blocked";
    note = "цена не обновляется";
    hint = `последняя цена помечена «${blocked}», и её метка времени стоит на месте`;
  }
