...
const state = {
  ...
  alertError: null,
  alertsSentAt: 0,
  // Таймеры «догнать подтверждение» после записи правила; снимаются блокировкой.
  alertChaseTimers: [],
...
// Когда спросить живой слой после записи правила, не дожидаясь очередного тика.
// Подтверждение рождается на сервере: агент перечитывает правила своим циклом и
// выкладывает состояние следующим снимком. Вкладка узнаёт об этом опросом раз в
// двадцать секунд, и на ровном месте набегала почти минута ожидания у чипа, который
// на самом деле был поставлен через пятнадцать секунд. Несколько коротких запросов
// вместо одного длинного ожидания стоят трёх обращений к бакету и убирают эту минуту.
const ALERT_CONFIRM_CHASE_MS = [4_000, 10_000, 18_000, 28_000];
...
/**
 * Догнать подтверждение только что записанного правила.
 *
 * Ждать своего двадцатисекундного тика незачем: подтверждение появляется на сервере
 * через его собственный цикл, и несколько коротких запросов после записи ловят его
 * почти сразу. Таймеры складываются в состояние, чтобы «Закрыть» их снял: иначе
 * запрос вернулся бы в уже очищенную вкладку.
 */
function chaseAlertConfirmation() {
  cancelAlertChase();
  state.alertChaseTimers = ALERT_CONFIRM_CHASE_MS.map((delay) => window.setTimeout(() => {
    if (state.cryptoKey && state.liveTimer) refreshLiveQuotes();
  }, delay));
}

function cancelAlertChase() {
  for (const timer of state.alertChaseTimers || []) window.clearTimeout(timer);
  state.alertChaseTimers = [];
}
...
/**
 * «17 мин», «4 ч», «3 дн» — возраст события в одну короткую подпись.
 *
 * Округление всегда ВНИЗ, до целой единицы: подпись стоит в чипе рядом с ценой, и
 * «4 ч» там честнее, чем «4.3 ч», а «через минуту станет 5 ч» никого не интересует.
 * Ноль и будущее дают null, а не «0 мин»: часы провайдера и браузера расходятся на
 * секунды, и отрицательный возраст — это рассинхрон, а не новость.
 */
...
function alertChipMarkup(rule) {
  ...
  // Словарь короткий и предметный, как у всех, кто это делает: активен — сработал —
  // остановлен. Прежние формулировки описывали процесс вместо состояния и читались
  // как жалобы: «проверяется на сервере» звучало как «застрял в очереди», а не
  // «работает», а «сервер не подтвердил проверку» — как отказ, хотя описывало
  // нормальное ожидание длиной в один тик. Подпись отвечает на вопрос «оно работает?»,
  // подробности уезжают в подсказку.
  let tone = "is-armed";
  let note = "активен";
  let hint = "сервер сверяет правило с каждой котировкой";
  if (pending) {
    tone = "is-pending";
    note = "сохраняем…";
    hint = "правило уходит на сервер";
  }
  else if (failed) {
    tone = "is-failed";
    note = "не сохранён — повторить";
    hint = "запись не дошла до хранилища; правило не поставлено";
  }
  else if (!status) {
    // Правило записано, но состояния по нему сервер ещё не прислал. Это ДВА разных
    // случая, и разница между ними — время. Обычный: правило только что записано, а
    // подтверждение приходит следующим снимком живого слоя — секунды. Ненормальный:
    // подтверждения нет и через полторы минуты, то есть движок правило не считает —
    // ровно то, что 6 августа длилось девять часов, пока чипы выглядели рабочими.
    //
    // Ветка стоит ПОСЛЕ `failed`: неудачная запись тоже приходит сюда без состояния,
    // и у неё есть свой, более точный ответ — «повторить».
    const waiting = state.alertsSentAt
      && Date.now() - state.alertsSentAt < ALERT_SYNC_GRACE_MS;
    if (waiting) {
      tone = "is-pending";
      note = "ставим на сервере…";
      hint = "сервер подтвердит правило ближайшим обновлением котировок";
    } else {
      tone = "is-blocked";
      note = "сервер не отвечает";
      hint = "правило записано, но сервер не сообщает, что проверяет его";
    }
  }
    ...
    tone = "is-failed";
    note = "уведомление не дошло";
    hint = "правило сработало, но сообщение не доставлено; сервер повторяет";
  }
  else if (fired) {
    tone = "is-fired";
    note = `сработал ${formatDateShort(fired, "time")}`;
    hint = status?.firedPrice ? `цена дошла до ${status.firedPrice}` : "правило сработало";
  }
    ...
    tone = "is-blocked";
    note = "нет котировки";
    hint = "по этому инструменту цена не приходит — проверять нечем";
  }
  // «delayed» здесь не появляется: отложенная котировка — настоящая цена, и правило
  // по ней срабатывает.
  //
  // Дальше — три РАЗНЫЕ причины, по которым правило сейчас не проверяется, и раньше
  // все три назывались «цена не обновляется». Фраза описывает поломку, а поломкой
  // был один случай из трёх: на неликвидной ADR и на закрытой ночью бирже чип горел
  // тревожным жёлтым при полностью исправной системе. Чип, который кричит без
  // повода, обесценивает и тот случай, когда повод есть.
  //
  // Разделяет их сервер (alerts._blocked_reason), а не страница: только у него есть
  // весь снимок, а «жив ли фид» — вопрос ко всему снимку, а не к одной строке.
  else if (blocked === "market-closed") {
    // Ночь — не отказ. Тон нейтральный намеренно.
    tone = "is-waiting";
    note = "биржа закрыта";
    hint = "площадка не торгует; правило проверится с открытием сессии";
  }
  else if (blocked === "thin-trading") {
    // Площадка открыта, фид отвечает, сделок нет. Возраст последней сделки — это и
    // есть вся новость, поэтому он стоит в подписи, а не прячется в подсказке.
    const ago = timeAgoShort(status?.lastMarketTime);
    tone = "is-waiting";
    note = ago ? `сделок нет ${ago}` : "сделок нет";
    hint = "по бумаге редко торгуют; правило сработает на первой же сделке за уровнем";
  }
  else if (blocked) {
    // «feed-silent» и всё, чего страница не знает: незнакомую причину безопаснее
    // показать тревожной, чем спрятать под успокаивающей формулировкой.
    tone = "is-blocked";
    note = "цена не обновляется";
    hint = "провайдер не присылает свежую цену — это уже неисправность, а не тишина рынка";
  }
  ...
  return `
    <span class="alert-chip ${tone}" data-alert-id="${escapeHtml(rule.id)}" title="${escapeHtml(hint)}">
...
async function commitRules(panel, rules, revert) {
  ...
  try {
    ...
    state.alertError = null;
    // Не ждать своего тика: подтверждение приходит от сервера, и его стоит спросить.
    chaseAlertConfirmation();
  } catch (error) {
