...
function alertChipMarkup(rule) {
  ...
  if (pending) { tone = "is-pending"; note = "сохраняется…"; }
    ...
    // Правило доехало до сервера, но состояния по нему сервер не прислал — значит движок
    // его не считал. Это ветка ПОСЛЕ `failed`: неудачная запись тоже приходит сюда без
    // состояния, и у неё есть свой, более точный ответ — «повторить».
    //
    // До этой ветки такой чип выглядел ровно как вооружённый, и это тот самый случай,
    // ради которого здесь вообще заведены состояния: 6 августа переезд живого слоя сломал
    // чтение правил, движок девять часов не проверял ничего, а чипы всё это время
    // выглядели рабочими. Отличать «сервер следит» от «сервер промолчал» важнее, чем
    // показать зелёное.
    tone = "is-blocked";
    note = "сервер не подтвердил проверку";
  }
